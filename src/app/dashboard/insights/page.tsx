...
export default async function InsightsPage() {
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 16px", display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "var(--fg-4)" }}>
            Motor de psicología financiera — sesgos cognitivos e insights de inversión
          </p>
        </div>
      ...
      {loadError && (
        <Callout color="neg" text={loadError} />
      )}
      ...
      {analysis && analysis.totalClosedTrades === 0 && (
        <div style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: 12, padding: 32, textAlign: "center" }}>
          <p style={{ margin: 0, fontSize: 14, color: "var(--fg-5)" }}>
            Aún no tienes operaciones cerradas. Importa tu historial de DEGIRO o registra ventas para ver los insights.
          </p>
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 10 }}>
            <KpiCard
              ...
              value={String(analysis.totalClosedTrades)}
            />
            <KpiCard
            ...
            <KpiCard
              ...
              fg={
                analysis.profitFactor == null ? "#4ADE80"
                  : analysis.profitFactor >= 2 ? "#4ADE80"
                  : analysis.profitFactor >= 1 ? "#FCD34D"
                  : "#F87171"
              }
              hint={
                analysis.profitFactor == null ? "Sin pérdidas registradas"
                  : analysis.profitFactor >= 2 ? "Excelente"
                  : analysis.profitFactor >= 1 ? "Positivo — margen mejorable"
                  : "Por debajo de 1 — las pérdidas superan las ganancias"
              }
            />
            <KpiCard
          ...
          <Section title="Benchmark S&P 500" icon="📊">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
              <MetricCell
                label="Tu cartera (realizado)"
                value={`${analysis.sp500.portfolioReturnPct >= 0 ? "+" : ""}${analysis.sp500.portfolioReturnPct.toFixed(2)}%`}
                fg={analysis.sp500.portfolioReturnPct >= 0 ? "#4ADE80" : "#F87171"}
              />
              <MetricCell
                label="S&P 500 simulado (SPY)"
                value={`${analysis.sp500.spyReturnPct >= 0 ? "+" : ""}${analysis.sp500.spyReturnPct.toFixed(2)}%`}
                fg="#93C5FD"
              />
              <MetricCell
                label="Capital analizado"
                value={fmtUSD(analysis.sp500.totalInvested)}
              />
              <MetricCell
            ...
            <Callout
              ...
              text={
                analysis.sp500.beatingMarket
                  ? `Tu stock-picking supera al S&P 500 en ${analysis.sp500.alphaPct.toFixed(2)} puntos porcentuales sobre el capital analizado.`
                  : `El S&P 500 (ETF SPY) habría rendido ${Math.abs(analysis.sp500.alphaPct).toFixed(2)}pp más que tu stock-picking en el mismo período con el mismo capital. Considera si la selección activa de valores justifica el esfuerzo.`
              }
            />
          </Section>
          ...
          {/* ── Disposition Effect ── */}
          <Section
            title="Efecto Disposición (Sesgo de Retención)"
            icon={analysis.disposition.biasDetected ? "⚠️" : "✓"}
            highlight={analysis.disposition.biasDetected ? "neg" : "pos"}
          >
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
              <MetricCell
                label="Días medios en ganadoras"
                value={`${analysis.disposition.avgDaysWinners}d`}
                fg="#4ADE80"
              />
              <MetricCell
                label="Días medios en perdedoras"
                value={`${analysis.disposition.avgDaysLosers}d`}
                fg={analysis.disposition.biasDetected ? "#F87171" : "var(--fg-2)"}
              />
            </div>
            <Callout
              color={analysis.disposition.biasDetected ? "neg" : "pos"}
              text={analysis.disposition.insight}
            />
          </Section>

          {/* ── Premature Sales ── */}
          {analysis.prematureSales.length > 0 && (
            <Section title="Ventas Prematuras (Alpha perdido)" icon="📉" highlight="warn">
              <p style={{ margin: "0 0 12px", fontSize: 13, color: "var(--fg-4)" }}>
                Posiciones vendidas cuyo precio de mercado actual supera en más de un 10% tu precio de venta:
              </p>
                ...
                {analysis.prematureSales.map((ps) => (
                  <div
                    ...
                    style={{
                      ...
                      background: "var(--card-inner)", border: "1px solid var(--card-border-inner)",
                  ...
                  >
                    <div>
                      ...
                      <span style={{ fontSize: 12, color: "var(--fg-5)", marginLeft: 8 }}>
                        Vendiste a {fmtUSD(ps.sellPrice)} · Ahora cotiza a {fmtUSD(ps.currentPrice)}
                      </span>
                    </div>
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#F87171" }}>
                        -{fmtUSD(ps.missedProfit)}
                      </div>
                      <div style={{ fontSize: 11, color: "var(--fg-5)" }}>
                        +{ps.missedProfitPct.toFixed(1)}% no capturado
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            {analysis.topDecisions.length > 0 && (
              <Section title="Mejores decisiones" icon="🏆">
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {analysis.topDecisions.map((d, i) => (
                    <DecisionCard key={i} entry={d} rank={i + 1} />
                  ))}
                </div>
              </Section>
            )}
            {analysis.worstMistakes.filter((d) => d.profitAmt < 0).length > 0 && (
              <Section title="Mayores errores" icon="❌">
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {analysis.worstMistakes.filter((d) => d.profitAmt < 0).map((d, i) => (
                    <DecisionCard key={i} entry={d} rank={i + 1} />
                  ))}
                </div>
              </Section>
      ...
      <div style={{ textAlign: "center", paddingTop: 8 }}>
        <Link
          ...
          style={{
            display: "inline-block", fontSize: 13, color: "var(--fg-4)",
            textDecoration: "none", borderBottom: "1px dashed var(--card-border)", paddingBottom: 1,
          }}
        >
...
function KpiCard({ label, value, fg = "var(--fg-1)", hint }: {
  label: string; value: string; fg?: string; hint?: string;
}) {
  return (
    <div style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: 12, padding: "12px 14px" }}>
      ...
      <p style={{ margin: 0, fontSize: 22, fontWeight: 700, color: fg }}>{value}</p>
...
function Section({
  ...
  const borderColor = highlight === "pos"
    ? "rgba(16,163,74,.35)"
    : highlight === "neg"
    ? "rgba(239,68,68,.35)"
    : highlight === "warn"
  ...
  return (
    <div style={{ background: "var(--card-bg)", border: `1px solid ${borderColor}`, borderRadius: 12, padding: 16 }}>
...
function Callout({ color, text }: { color: "pos" | "neg" | "warn" | "info"; text: string }) {
  const styles = {
    pos:  { bg: "rgba(16,163,74,.1)",   bd: "rgba(16,163,74,.35)",   fg: "#4ADE80" },
    neg:  { bg: "rgba(239,68,68,.1)",   bd: "rgba(239,68,68,.35)",   fg: "#F87171" },
    warn: { bg: "rgba(245,158,11,.1)",  bd: "rgba(245,158,11,.3)",   fg: "#FCD34D" },
    info: { bg: "rgba(37,99,235,.12)",  bd: "rgba(37,99,235,.35)",   fg: "#93C5FD" },
  }[color];

  return (
    <div style={{ background: styles.bg, border: `1px solid ${styles.bd}`, borderRadius: 8, padding: "9px 12px", fontSize: 13, color: styles.fg, lineHeight: 1.5 }}>
      {text}
    </div>
  );
}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 11, color: "var(--fg-5)" }}>
            {entry.holdingDays}d · {entry.date}
          </p>
        </div>
      </div>
      <div style={{ textAlign: "right", flexShrink: 0 }}>
        <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: isPos ? "#4ADE80" : "#F87171" }}>
          {isPos ? "+" : ""}{fmtUSD(entry.profitAmt)}
        </p>
        <p style={{ margin: 0, fontSize: 11, color: isPos ? "#4ADE80" : "#F87171" }}>
          {isPos ? "+" : ""}{entry.profitPct.toFixed(2)}%
        </p>
      </div>
    </div>
  );
