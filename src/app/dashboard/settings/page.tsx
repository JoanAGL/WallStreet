...
export default function SettingsPage() {
  ...
  useSession(); // keep session context
  ...
  async function resetPortfolio() {
    if (resetConfirm !== "LIMPIAR") { setResetMsg({ ok: false, text: 'Escribe "LIMPIAR" para confirmar.' }); return; }
    setResetLoading(true); setResetMsg(null);
    const res = await fetch("/api/portfolio/reset", { method: "DELETE" });
    setResetLoading(false);
    if (res.ok) {
      setResetMsg({ ok: true, text: "Cartera eliminada. Redirigiendo al dashboard…" });
      setResetConfirm("");
  ...
  return (
    <div className="space-y-6 max-w-lg">
      ...
      <Section title="Limpiar cartera">
        <p className="text-sm text-gray-600">
          Elimina <strong>todas tus acciones, transacciones, historial de ventas y análisis</strong> sin borrar tu cuenta. Útil para empezar desde cero o reimportar un CSV corregido.
        </p>
        <div className="space-y-3">
          <Field label='Escribe "LIMPIAR" para confirmar'>
            <input
              ...
              placeholder="LIMPIAR"
              className={`${inputClass} border-orange-200 focus:ring-orange-400`}
            />
          </Field>
          {resetMsg && (
            <p className={`text-sm ${resetMsg.ok ? "text-green-600" : "text-red-600"}`}>{resetMsg.text}</p>
          )}
          <button
            type="button"
            onClick={resetPortfolio}
            disabled={resetLoading || resetConfirm !== "LIMPIAR"}
            style={{ background: resetLoading || resetConfirm !== "LIMPIAR" ? undefined : "#EA580C" }}
