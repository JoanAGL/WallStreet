...
### Clasificador de Régimen de Mercado (Hurst + GARCH + RSI)

`classifyRegime` en `quantitativeService.ts` cruza cuatro señales ya calculadas por el pipeline — exponente de Hurst (persistencia), RSI14 (momentum), volumen relativo (actividad) y volatilidad condicional GARCH(1,1) anualizada — para clasificar cada activo en uno de seis regímenes, a coste de cómputo casi nulo:

| Régimen | Condición | Sesgo inyectado en Gemini |
|---------|-----------|---------------------------|
| `HIGH_VOLATILITY` | GARCH anualizada > 40% o volumen relativo > 3× | Amplía rangos objetivo y dispersión entre escenarios |
| `VOLATILITY_CRUSH` | GARCH anualizada < 15% | Estrecha rangos objetivo; baja dispersión |
| `TRENDING_BULL` | Hurst > 0.6 y RSI > 55 | Sesga escenario POSITIVO en corto plazo |
| `TRENDING_BEAR` | Hurst > 0.6 y RSI < 45 | Sesga escenario NEGATIVO en corto plazo |
| `MEAN_REVERTING` | Hurst < 0.45 | El escenario NEUTRAL tiene el mayor peso estadístico |
| `RANDOM_WALK` | resto | Escenarios equiprobables; intervalos de confianza más anchos |

El régimen se persiste en `StockAnalysis.marketRegime` y se inyecta por activo en el payload de Gemini como `regime: { state, bias }` (~15 tokens), con la tabla de sesgos documentada en la instrucción estática del sistema. `classifyRegimeSafe` tolera datos ausentes: sin Hurst o RSI devuelve null (régimen no interpretable); volumen ausente se asume normal y GARCH ausente zona neutra.
...
- **Simulación Monte Carlo** — 1 000 trayectorias GBM, horizontes configurables (1–30 años), aportaciones mensuales opcionales; distribución t de Student df=5 por defecto con el shock **escalado a varianza unitaria** (`√((df−2)/df)`) para que la volatilidad realizada coincida con la sigma estimada y la corrección de Itô sea consistente (colas gordas preservadas → VaR de cola más conservador que la normal sin sesgar la mediana). La sigma de entrada es la **volatilidad de la cartera** (retornos diarios ponderados por valor, con efecto diversificación), no la media de las acciones individuales; la página de simulación muestra tarjetas explícitas de **VaR95** (pérdida máxima probable al 95%) y **CVaR95** (Expected Shortfall: media de pérdidas en el peor 5% de escenarios), más probabilidad de pérdida; percentiles p10/p25/p50/p75/p90 en gráfico de abanico. Parámetros `distribution` y `degreesOfFreedom` configurables via API para comparativa NORMAL vs STUDENT_T
...
- **Soporte multi-divisa genérico** — Las acciones extranjeras (SAB.MC en EUR, NOVO-B.CO en DKK, LSE en GBp…) detectan su divisa nativa vía `meta.currency` de Yahoo Finance. El precio se almacena en divisa nativa en `StockAnalysis.price` y se convierte a USD en `priceUSD` con el tipo de cambio **de su propia divisa** (`fetchFxToUSD`: `{CUR}USD=X` con caché 1h; GBp/GBX → GBPUSD/100). Si el FX no está disponible, la cotización falla explícitamente y se conserva el último precio bueno (nunca se valora con un tipo equivocado ni se machaca con 0). El `StockCard` muestra `€X.XX (~$X.XX)` o `X.XX DKK (~$X.XX)` y el «Nivel ref.» de la señal va en la divisa de cotización. Cotización con doble fuente: Finnhub primero y fallback a Yahoo ante **cualquier** fallo (403/429/timeout o precio 0). Todas las vistas agregadas usan `priceUSD`
...
- **Sistema de transacciones (WAC)** — Motor completo de registro de compras y ventas por posición usando el método de Coste Medio Ponderado (Weighted Average Cost). Cada transacción almacena: tipo (BUY/SELL), acciones, precio, fecha (opcional) y notas (opcional). El servicio calcula en tiempo real: precio medio WAC dinámico (recalculado con cada BUY en orden cronológico), precio medio de venta, coste base abierto, valor actual, PnL no realizado ($ y %), PnL realizado ($ y %), precio de equilibrio, días en cartera (desde primera transacción), rentabilidad anualizada CAGR y peso en cartera (%). API: `POST /api/portfolio/transactions` (registrar BUY/SELL), `GET /api/portfolio/transactions?stockId=xxx&currentPrice=yyy` (métricas completas), `PATCH /api/portfolio/transactions/[id]` (editar transacción), `DELETE /api/portfolio/transactions/[id]` (eliminar). Panel «Transacciones» plegable en cada StockCard con formulario inline (BUY/SELL/SPLIT toggle), métricas compactas y lista de operaciones con edición y eliminación
- **Comisiones y dividendos** — Campo `fee` en cada transacción: en BUY se capitaliza en el coste base (criterio fiscal), en SELL reduce el importe obtenido (P&L realizado neto). Nuevo tipo `DIVIDEND`: `shares` × `price` = importe bruto y `fee` = retención; se acumula en «Dividendos netos» sin alterar la posición, reduce el precio de equilibrio y entra en el CAGR como riqueza final. El importador DEGIRO parsea la columna «Costes de transacción» (convertida a USD con el tipo de cambio de la fila) y suma las comisiones de ejecuciones parciales fusionadas. Formulario con toggle Compra/Venta/Split/Dividendo y campos de comisión/retención opcionales
- **Splits y contrasplits** — Transacción de tipo `SPLIT` cuyo campo `shares` guarda el factor (10 = split 10:1, 0.1 = contrasplit 1:10) y `price` se almacena como 1 (sin significado). Al procesarse cronológicamente multiplica las acciones abiertas y divide el WAC, dejando invariantes el capital invertido y el P&L realizado; un split sobre posición cerrada es un no-op y no altera los días en cartera. Soportado en todos los motores que recorren transacciones (`calculatePositionMetrics`, `getTransactionHistory`, análisis de decisiones e importador DEGIRO). Caso de uso: el split 10:1 de NFLX (nov 2025) — registrar el split corrige el WAC sin tocar las transacciones pre-split
- **Detección automática de splits** — En cada actualización de datos, `splitDetectionService` consulta los splits corporativos del ticker en Yahoo (`chart?events=splits`, 10 años, caché 24h) y registra las transacciones `SPLIT` pendientes con nota «Auto-detectado». Salvaguardas: solo eventos posteriores a la primera compra con posición abierta; dedupe ±7 días frente a splits ya registrados; y **corroboración obligatoria** — el WAC en la fecha del split dividido por el factor debe ser coherente (ratio 0,5–2×) con el precio medio de las compras post-split o con el precio de mercado actual. Sin referencia no se auto-aplica (un historial ya ajustado no se multiplica por error). Las transacciones auto-detectadas se eliminan desde el panel y el WAC se recalcula
- **Sincronización de posición** — `Stock.quantity` y `purchasePrice` se recalculan desde el WAC tras cada alta/edición/borrado de transacción (`syncStockPosition`), manteniendo coherentes el resumen del dashboard y la vista de Rendimiento
...
- **Curva de evolución y TWR** — Snapshot diario del valor de cartera (`PortfolioSnapshot`: valor de mercado USD, coste base WAC y flujos externos netos del día), capturado tras cada actualización de datos y en el cron diario (idempotente por usuario+día). `/dashboard/portfolio` muestra la curva valor vs. coste (SVG sin dependencias) con **Time-Weighted Return** encadenado — `r_i = (V_i − F_i)/V_{i−1} − 1` — que neutraliza aportaciones y retiradas para medir rentabilidad real, y su anualización a partir de 30 días de historia. La curva se construye a razón de un punto por día desde su activación
...
- Los precios de SPY en las fechas de entrada y salida provienen del **histórico diario real de Yahoo** (3 años, caché 24h en `CacheEntry`); la tabla interpolada 2014-2026 queda solo como fallback. La comparación es del **mismo período de tenencia** de cada operación (compra en `firstBuyDate`, venta en `sellDate`), no contra SPY mantenido hasta hoy
